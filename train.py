...
from utils import load_fashion_mnist, load_mnist, one_hot, train_val_split, compute_accuracy
...
def train(config=None):
    ...
    with wandb.init(config=config):
        ...
        dataset = cfg.get('dataset', 'fashion_mnist')
        
        # meaningful name for the run (assignment asks for this)
...
def main():
    ...
    parser.add_argument('--wandb_project', type=str, default='atri-assignment')
    
    args = parser.parse_args()
    config = vars(args)
    proj = config.pop('wandb_project')
    
    # init wandb and train
    ...
    for ep in range(epochs):
        ...
        wandb.log({
            'epoch': ep+1, 'train_loss': avg_loss, 'val_loss': vl_loss,
            'train_acc': tr_acc, 'val_acc': vl_acc,
        })
